    ...
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        {isRouterLink(items?.[0]?.href) ? (
          ...
          >
            <Image src={logo} alt={logoAlt} width={28} height={28} ref={logoImgRef} priority />
          </Link>
        ) : (
          <a
            className="pill-logo"
              ...
              <li key={item.href || `item-${i}`} role="none">
                {isRouterLink(item.href) ? (
                  <Link
                    role="menuitem"
                    href={item.href}
                    className={`pill${isActiveLink(item.href, i) ? ' is-active' : ''}`}
      ...
      <div id="primary-mobile-menu" className="mobile-menu-popover mobile-only" ref={mobileMenuRef} style={cssVars}>
